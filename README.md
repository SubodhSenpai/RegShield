# RegressionShield 🛡️
### Agentic AI Reasoning & Execution Trace Evaluation Framework
...
│  • Composite Trace Index (Unified 0.0 – 1.0 quality gate)                   │
│                                                                             │
...
│                                                                             │
│  [ Dynamic Local Web Dashboard & REST Ingestion Server ]                    │
│  • Local HTTP Server: regshield serve --port 8000                            │
...
| **Trace Efficiency** | Heuristic Loop Detector | Detects agent oscillations, duplicate queries, and cycle loops. Penalizes excessive exploration steps. |
...
| **Composite Trace Score** | Weighted Index | Unified 0.0 - 1.0 quality gate index combining tools, args, order, and reasoning. |
...
### 1. Direct Execution Trace Evaluation & Failure Diagnosis
```python
...
```bash
# 1. Evaluate agent execution traces from any JSON file:
...
### External Ingestion REST API (`POST /api/evaluate-trace`)
Any agent running anywhere (Python, Node.js, Go, cURL) can post execution traces directly into RegressionShield with custom API keys and models:
...
Run all unit and trace evaluation tests (100% pass rate, offline & deterministic):
```bash
pytest -v tests/
```
