"""Core Agent Trajectory and Tool Calling Evaluation Metrics.

Implements rigorous component-level evaluation for Agentic AI execution paths,
reflecting industry standards (DeepEval, Arize AX / Phoenix):
- Tool Selection (Precision / Recall / F1)
- Argument Correctness (Schema & Value Verification)
- Call Ordering (Sequential Dependency Enforcement)
- Step Efficiency & Loop Detection (Thrashing & Redundancy Penalties)
- Reasoning Step Faithfulness (Thought-Observation Alignment)
...
logger = logging.getLogger("regression_shield.agent_metrics")
...
class ArgumentCorrectnessMetric:
    ...
    def evaluate(expected_args: Dict[str, Dict[str, Any]], trajectory: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Map each tool name to all argument dicts it was called with
        ...
        for tool_name, exp_params in expected_args.items():
            ...
            # Best-match across invocations of this tool
            best_match_hits = -1
            best_mismatches = []
            ...
            for candidate_args in calls_for_tool:
                ...
                for param_k, param_v in exp_params.items():
                    ...
                    if actual_v is not None:
                        # 1. Numeric equality check (e.g. 4500 vs 4500.0)
                        try:
                        ...
                        # 2. Normalized string check (case-insensitive, underscore/hyphen agnostic)
                        if not is_match:
...
class ToolCallOrderMetric:
    ...
    @staticmethod
    def evaluate(expected_order: List[str], invoked_tools: List[str]) -> Dict[str, Any]:
        ...
        violations = []
        # Find earliest index of each tool
        indices: Dict[str, int] = {}
        for idx, tool in enumerate(invoked_tools):
            if tool not in indices:
                indices[tool] = idx

        # Support either list of pairs [["A", "B"], ["B", "C"]] or sequential list ["A", "B", "C"]
        pairs = []
...
class StepEfficiencyMetric:
    ...
    def evaluate(trajectory: List[Dict[str, Any]], optimal_steps: int = 3) -> Dict[str, Any]:
        ...
        # Step count inflation penalty compared to optimal baseline
    ...
    def evaluate(trajectory: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for step in trajectory:
            ...
            if thought:
                ...
                # Anomaly: Observation contained error/policy violation, but thought claimed success
